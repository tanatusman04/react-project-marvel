...
function App() {
  ...
  useEffect(()=>{
      const fetch = async()=>{
        if(query===''){
          // checking if favorites array is empty or does not exist таңдаулылар массивінің бос немесе жоқ екенін тексеру
          // кілт параметр ретінде Сақтау интерфейсінің getItem() әдісіне берілсе, әдіс көрсетілген кілт арқылы жадта сақталған мәнді қайтарады.
